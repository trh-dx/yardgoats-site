    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
      {sponsors.map((pkg) => {
        ...
        return (
          ...
          >
            {/* Top stripe */}
            <div className="rounded-t-xl" style={{ height: s.stripeH, background: s.stripe }} />
            ...
            {s.badge && (
              <span
                className="absolute left-1/2 -translate-x-1/2 font-inter font-bold text-[0.62rem] tracking-[2px] uppercase px-3 py-1 rounded-b-md whitespace-nowrap z-10"
                style={{
                  top: s.stripeH,
            ...
            <div className={`relative px-6 pb-7 ${s.padTop}`}>
              ...
              >
                {pkg.price}
                {pkg.priceSuffix && <span style={{ fontSize: s.priceSufSize }}>{pkg.priceSuffix}</span>}
              </div>
              <div className="text-center text-[0.71rem] mb-0" style={{ color: "rgba(167,168,170,0.55)" }}>
              ...
              <ul className="space-y-0 mb-6">
                {pkg.perks.map((perk) => (
                  ...
                  >
                    <span className="font-bold mt-0.5 shrink-0 text-[0.78rem]" style={{ color: s.accent }}>✓</span>
                    {perk}
                  </li>
                ))}
              </ul>
              ...
              <a
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.backgroundColor = s.btnHoverBg; }}
                onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.backgroundColor = s.btnBg; }}
              >
                {pkg.cta}
              </a>
            </div>
